...
Facilities are subject to inspections on a regular basis in terms of some health and sanitary criteria. We will examine how the inspection results vary depending on previous results. For example, are restaurants more likely to perform better after they are given a "pass with conditions"? 

### Does the type of a given facility influence the outcome of its inspection?
From gyms to restaurants, there are different types of facilities which serve food in the city of Chicago. This question mainly asks if some types of facilities are riskier than others and if so how risky are they? Moreover, how homogenous is the sanitary status of facilities which belong to a given type? Moreover, what are the most common violations? Are there violations which typically happen in most facilities which fail inspections? Or do they depend on another factor such as establishment type or location?
...
Restaurants in Chicago offer a wide range of cuisines. Is there some form of relationship between the cuisine of a restaurant and the outcome of its inspection?
